...
describe API::MergeRequests, api: true  do
  ...
  describe "GET /projects/:id/merge_requests/:merge_request_id" do
    it 'exposes known attributes' do
      ...
      expect(json_response['merge_when_merge_when_pipeline_succeeds']).to be_falsy
  ...
  describe "PUT /projects/:id/merge_requests/:merge_request_id/merge" do
    ...
    it "enables merge when pipeline succeeds if the pipeline is active" do
      ...
      put v3_api("/projects/#{project.id}/merge_requests/#{merge_request.id}/merge", user), merge_when_merge_when_pipeline_succeeds: true

      expect(response).to have_http_status(200)
      expect(json_response['title']).to eq('Test')
      expect(json_response['merge_when_merge_when_pipeline_succeeds']).to eq(true)
    end
  end
